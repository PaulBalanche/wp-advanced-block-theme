...
class Css
{
     ...
     *
     */
    public static function generate_coffeekraken_css_layout(
        $cells,
        $device = "desktop"
    ) {
        ...
        foreach ($cells as $cell) {
            $columnStart =
                isset($cell[$device]) &&
                is_array($cell[$device]) &&
                isset($cell[$device]["columnStart"])
                    ? $cell[$device]["columnStart"]
                    : 1;
            $width =
                isset($cell[$device]) &&
                is_array($cell[$device]) &&
                isset($cell[$device]["width"])
                    ? $cell[$device]["width"]
                    : 1;
            $rowStart =
                isset($cell[$device]) &&
                is_array($cell[$device]) &&
                isset($cell[$device]["rowStart"])
                    ? $cell[$device]["rowStart"]
                    : 1;
            $height =
                isset($cell[$device]) &&
                is_array($cell[$device]) &&
                isset($cell[$device]["height"])
                    ? $cell[$device]["height"]
                    : 1;
            ...
            while ($column_index <= $countColumns) {
                foreach ($cells as $key => $cell) {
                    ...
                    if (
                        ...
                        $columnStart + $width - 1 < $column_index
                    ) {
                        continue;
                    }

                    $css_row[$column_index - 1] = $key;
                }
