...
export class Devices {
    ...
    addComponent(componentInstance) {
        this.componentInstance[componentInstance.props.clientId] =
            componentInstance;
    ...
    getButtonGroup() {
        const minText =
            this.getMediaQueries()[this.currentDevice]["minWidth"] != null &&
            this.getMediaQueries()[this.currentDevice]["minWidth"] > 0 ? (
                <>
                    {this.getMediaQueries()[this.currentDevice]["minWidth"] +
                        "px"}
                    <Dashicon icon="arrow-left-alt2" />
                </>
            ) : null;
        const maxText =
            this.getMediaQueries()[this.currentDevice]["maxWidth"] != null ? (
                <>
                    <Dashicon icon="arrow-left-alt2" />
                    {this.getMediaQueries()[this.currentDevice]["maxWidth"] +
                        "px"}
                </>
            ) : null;
        ...
        return (
            <>
                <div className="devicesInfo">
                    {minText}
                    {this.currentDevice.charAt(0).toUpperCase() +
                        this.currentDevice.slice(1)}
                    {maxText}
                </div>
                <ButtonGroup
