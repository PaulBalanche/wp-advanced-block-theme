...
class CliCommand extends ServiceBase
{
    ...
    {
        $custom_blocks_routing = $this->get_config()->get_spec(
        ...
        if (is_array($front_components) && count($front_components) > 0) {
            ...
            foreach ($front_components as $component) {
                ...
                $component_frontspec = $this->frontEndService->get_component_viewspec(
                    $component
                );
                // If invalid or null component, just bypass it and continue to the next component
                if (
                    !is_null($component_frontspec) &&
                    is_array($component_frontspec) &&
                    isset(
                        $component_frontspec["id"],
                        $component_frontspec["path"]
                    )
                ) {
                    if (
                        $custom_blocks_routing &&
                        is_array($custom_blocks_routing) &&
                        array_key_exists(
                            $component_frontspec["id"],
                            $custom_blocks_routing
                        )
                        ...
                        if (
                            ...
                            $coreBlockInstance = new CoreBlock(
                                $custom_blocks_routing[
                                    $component_frontspec["id"]
                                ]
                            );
                            $block_spec = $coreBlockInstance->generate_block_spec(
                                $component_frontspec
                            );
                        } elseif (
                            strpos(
                            ...
                            $layoutBlockInstance = new LayoutBlock(
                                str_replace(
                                    "layout/",
                                    "",
                                    $custom_blocks_routing[
                            ...
                            $block_spec = $layoutBlockInstance->generate_block_spec(
                                $component_frontspec
                            );
                        }
                    } else {
                        ...
                        $block_spec = $componentBlockInstance->generate_block_spec(
                            $component_frontspec
                        );
                    ...
                    if (isset($block_spec)) {
                        if ($block_spec) {
                            \WP_CLI::success(
                                $block_spec . " successfully generated."
                            );
                        } else {
                            \WP_CLI::error(
                                $component_frontspec["id"] .
                                    ": an error occurs during block spec generation..."
                            );
                        }
                    }
                    if (isset($block_metadata)) {
                        if ($block_metadata) {
                            \WP_CLI::success(
                                $block_metadata . " successfully generated."
                            );
                        } else {
                            \WP_CLI::error(
                                $component_frontspec["id"] .
                                    ": an error occurs during block metadata generation..."
                            );
                        }
                    }
                } else {
                    \WP_CLI::error(
                        $component .
                            ": an error occurs during component viewspec generation..."
                    );
                }
            }
        }
