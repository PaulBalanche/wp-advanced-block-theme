...
class ComponentBlock extends ModelBase
{
    ...
    public function get_block_spec()
    {
        if (is_null($this->blockSpec)) {
            $spec_json_file =
                $this->get_block_dir() .
                "/" .
                $this->get_config()->get("viewspecJsonFilename");
            if (file_exists($spec_json_file)) {
                $block_spec = json_decode(
                    file_get_contents($spec_json_file),
                    true
                );
    ...
    public function get_screenshot_src()
    {
        ...
        if (
            $screenshot_file &&
            is_array($screenshot_file) &&
            count($screenshot_file) > 0
        ) {
            $pathinfo_screenshot = pathinfo($screenshot_file[0]);
            return str_replace(
                home_url(),
                "",
                $this->get_block_dir_uri() .
                    "/" .
                    $pathinfo_screenshot["basename"]
            );
        }
    ...
    public function generate_block_spec($component_frontspec)
    {
        ...
        $default_parent =
            $container_spec &&
            is_array($container_spec) &&
            isset($container_spec["is_main"]) &&
            $container_spec["is_main"]
        ...
        $this->blockSpec = apply_filters(
            ...
            [
                ...
                "category" =>
                    ...
                    !empty($component_frontspec["category"])
                        ? [
                            "slug" => sanitize_title(
                                $component_frontspec["category"]
                            ),
                            "title" => $component_frontspec["category"],
                        ]
                        : [
                            "slug" => sanitize_title(
                                $this->get_config()->get(
        ...
        $block_spec_json_filename =
            $block_dir . "/" . $this->get_config()->get("viewspecJsonFilename");
    ...
    public function get_block_metadata_json_file()
    {
        $metadata_json_file =
            $this->get_block_dir() .
            "/" .
            $this->get_config()->get("blockMetadataJsonFilename");
    ...
    public function generate_block_metadata()
    {
        ...
        if (
            is_array($block_spec) &&
            isset($block_spec["props"]) &&
            is_array($block_spec["props"])
        ) {
            $metadata = [];
            ...
            foreach ($block_spec["props"] as $key_props => $val_prop) {
                if (is_array($val_prop) && isset($val_prop["type"])) {
                    $currentType =
                        isset($val_prop["repeatable"]) &&
                        $val_prop["repeatable"] == true
                            ? "array"
                            : strtolower($val_prop["type"]);
                    $currentType =
                        isset($val_prop["responsive"]) &&
                        $val_prop["responsive"] == true
                            ? "object"
                            : $currentType;
    ...
    public function get_override_viewspec()
    {
        $override_spec_file =
            $this->get_block_dir() .
            "/" .
            $this->get_config()->get("overrideSpecJsonFilename");
        if (file_exists($override_spec_file)) {
            $override_spec = json_decode(
                file_get_contents($override_spec_file),
                true
            );
    ...
    public function render()
    {
        ...
        if (is_array($block_spec)) {
            ...
            $render_attributes = apply_filters(
                'Abt\render_component_block_attributes',
                $render_attributes
            );
            $render_attributes = apply_filters(
            ...
            $render_attributes["anchor"] = Anchor::get(
            ...
            if ($this->validateProps($render_attributes, $content)) {
                ...
                if (
                    ...
                    return '<div class="alert">' .
                        $render_attributes["admin_error_message"] .
                        "</div>";
                }
            }
        }
    ...
    public function validateProps(
        ...
        if (
            ...
            isset($block_spec["props"]) &&
            is_array($block_spec["props"]) &&
            count($block_spec["props"]) > 0
        ) {
            foreach ($block_spec["props"] as $key_prop => $prop) {
                $propInstance = new Prop(
                    $key_prop,
    ...
    public function attributes_autosaves_post( $attributes, $post_id, $client_id, $content = '' )
    {
        if (
            !file_exists(
                $this->get_config()->get("componentBlocksAutoSaveLocation") .
                    $post_id .
                    "/" .
                    $this->get_ID()
            )
